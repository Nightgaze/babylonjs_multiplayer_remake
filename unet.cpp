...
typedef void (*CheckKeysFunc) ();
static CheckKeysFunc g_checkKeys_cb = NULL;
...
{
	g_logCB = f;
	Log("Set logging func. This should be the first message you see!!!");
}

extern "C" void SetCheckKeysFunc(CheckKeysFunc f)
{
	g_checkKeys_cb = f;
	Log("Set check keys func!!!");
}
...
extern "C" int UnetEncryptionDestroy()
{
	return 0;
	// This function is more interesting on other platforms.
}
